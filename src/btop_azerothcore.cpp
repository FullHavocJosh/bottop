...
namespace AzerothCore {
	...
	std::pair<bool, double> Query::check_rebuild_status() {
		...
		try {
			...
			std::string result = ssh_.execute(check_cmd);
	...
	std::vector<ContainerStatus> Query::fetch_container_statuses() {
		...
		try {
			...
			std::string result = ssh_.execute(cmd);
	...
	void init() {
		...
		debug_log << "Proceeding with SSH connection" << std::endl;
		std::cerr << "[BOTTOP DEBUG] init() proceeding with SSH connection" << std::endl;
		
		try {
			ssh_client = std::make_unique<SSHClient>(config.ssh_host);
			if (!ssh_client->connect()) {
				debug_log << "SSH connection failed: " << ssh_client->last_error() << std::endl;
				std::cerr << "[BOTTOP DEBUG] SSH connection failed: " << ssh_client->last_error() << std::endl;
				current_data.error = "Failed to connect: " + ssh_client->last_error();
				return;
			}
			
			debug_log << "SSH connected successfully" << std::endl;
			std::cerr << "[BOTTOP DEBUG] SSH connected, creating Query object" << std::endl;
			query = std::make_unique<Query>(*ssh_client, config);
			active = true;
	...
	bool check_server_online() {
		if (!ssh_client || !ssh_client->is_connected()) return false;
		
		try {
			// Check if container is running
			std::string cmd = "docker ps --filter name=" + config.container + " --format '{{.Status}}'";
			std::string status = ssh_client->execute(cmd);
	...
	void load_expected_values() {
		...
		// Try to load from remote config if available
		if (!ssh_client || !ssh_client->is_connected() || config.config_path.empty()) {
			debug_log << "No remote config available - ssh=" << (ssh_client ? "yes" : "no") 
			          << ", connected=" << ((ssh_client && ssh_client->is_connected()) ? "yes" : "no")
			          << ", config_path=" << config.config_path << std::endl;
			debug_log.close();
			Logger::error("load_expected_values: No remote config available, using defaults (ssh=" + std::string(ssh_client ? "yes" : "no") + ", connected=" + std::string((ssh_client && ssh_client->is_connected()) ? "yes" : "no") + ", config_path=" + config.config_path + ")");
			return;
		}
		...
		try {
			...
			std::string config_content = ssh_client->execute(cmd);
			...
			std::string bracket_content = ssh_client->execute(bracket_cmd);
