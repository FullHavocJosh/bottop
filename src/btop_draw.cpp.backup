...
namespace Draw {
	...
	std::vector<Draw::AzerothCore::DisplayItem> zone_display_list;
		...
		//* Container statuses - show always underneath Status line
		if (!data.containers.empty()) {
			// No blank line - containers appear immediately after Status
			
			// Determine if we should show helper containers (db-import, bot-relocate, client-data-init)
			// Only show helpers if worldserver is NOT running normally
			...
			for (const auto& container : data.containers) {
				...
				// Format: "  worldserver: Up 2 hours" with white container name
	...
	for (int clear_line = 0; clear_line < dist_height - 2; clear_line++) {
		...
		if (!data.factions.empty() and dist_cy < dist_y + dist_height - 1) {
			out += Mv::to(dist_cy++, dist_x + 2) + title + "Factions:";
		...
		if (!data.continents.empty() and dist_cy < dist_y + dist_height - 1) {
			out += Mv::to(dist_cy++, dist_x + 2) + title + "Continents:";
			...
			if (!data.levels.empty() and dist_cy < dist_y + dist_height - 1) {
				out += Mv::to(dist_cy++, dist_x + 2) + title + "Levels:";
				...
				if (bracket_defs.empty()) {
					bracket_names = {"1-9", "10-19", "20-29", "30-39", "40-49", "50-59", "60", "61-69", "70", "71-79", "80"};
				} else {
